...
class SearchByNameController extends Controller
{
        public function Search(Request $request)
        {
            ...
            if($request->lang == 'en'){
                $products = Product::select('title_en as title'  , 'id' , 'offer' , 'price_before_offer' , 'final_price' , 'offer_percentage' , 'category_id')->where('deleted' , 0)->where('hidden' , 0)->where('remaining_quantity', '>', 0)->Where(function($query) use ($search) {
                    $query->Where('title_en', 'like', '%' . $search . '%')->orWhere('title_ar', 'like', '%' . $search . '%');
                })->get(); 
            }else{
                $products = Product::select('title_ar as title'  , 'id' , 'offer' , 'price_before_offer' , 'final_price' , 'offer_percentage' , 'category_id')->where('deleted' , 0)->where('hidden' , 0)->where('remaining_quantity', '>', 0)->Where(function($query) use ($search) {
                    $query->Where('title_en', 'like', '%' . $search . '%')->orWhere('title_ar', 'like', '%' . $search . '%');
                })->get(); 
            }
