...
class RateController extends Controller
{
    ...
    public function addrate(Request $request){
        ...
        $rate->save();
        $orderItems = OrderItem::where("product_id", $request->product_id)->pluck("id")->toArray();
        $rates = Rate::whereIn('order_id' , $orderItems)->count('id');
        
        $sumrates = Rate::whereIn('order_id' , $orderItems)->sum('rate');
        
        if($rates > 0 ){
            $average = $sumrates / $rates;
        }else{
            $average = 0;
        }
