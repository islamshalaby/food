...
class FavoriteController extends Controller
{
    ...
    public function getfavorites(Request $request){
        ...
        if($request->lang == 'en'){
            $products = Product::whereIn('id', $favorites)->where('deleted' , 0)->where('hidden' , 0)->where('remaining_quantity', '>', 0)->select('id', 'title_en as title' , 'final_price' , 'price_before_offer' , 'offer' , 'offer_percentage' , 'category_id', 'rate' )->get();
        }else{
            $products = Product::whereIn('id', $favorites)->where('deleted' , 0)->where('hidden' , 0)->where('remaining_quantity', '>', 0)->select('id', 'title_ar as title' , 'final_price' , 'price_before_offer' , 'offer' , 'offer_percentage' , 'category_id', 'rate' )->get();
        }
