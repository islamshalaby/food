...
<script>
    ...
    $("tbody#sortable").sortable({
        ...
        update : function () {
            ...
            var url = "{{ route('sub_categories.sort') }}";
                ...
                <table id="without-print" class="table table-hover non-hover" style="width:100%">
                    ...
                    <tbody id="sortable">
                        ...
                        @foreach ($data['sub_categories'] as $sub_category)
                            <tr id="id_{{ $sub_category['id'] }}">
                                <td><?=$i;?></td>
                                <td>{{ App::isLocale('en') ? $sub_category->title_en : $sub_category->title_ar }}</td>
                                <td>
                                    <a href="{{ route('categories.details', $sub_category->category->id) }}">
                                        {{ App::isLocale('en') ? $sub_category->category->title_en : $sub_category->category->title_ar }}
                                    </a>
                                </td>
                                <td class="text-center blue-color"><a href="{{ route('sub_categories.details', $sub_category->id) }}" ><i class="far fa-eye"></i></a></td>
                                @if(Auth::user()->update_data) 
                                    <td class="text-center blue-color" ><a href="{{ route('sub_categories.edit', $sub_category->id) }}" ><i class="far fa-edit"></i></a></td>
                                @endif
                                @if(Auth::user()->delete_data) 
                                    <td class="text-center blue-color" >
                                        @if(count($sub_category->products) == 0)
                                        <a onclick='return confirm("{{ __('messages.are_you_sure') }}");' href="{{ route('sub_categories.delete', $sub_category->id) }}" ><i class="far fa-trash-alt"></i></a>
                                        @else
